    ...
    $scope.task1.questions = [
        {
            ...
            type: "checkbox",
            answers: ["Statistik", "Werbezwecke", "Jahresbericht", "Erhöhen der internen Sicherheit", "Öffentliche Meldungen", "Erfahrungsberichte", "Kontaktaufnahme"]
        }
    ];
    $scope.date = new Date();
    $scope.starttime = performance.now();
...
evalApp.controller('task2Ctrl', function ($scope, $location) {
    ...
    $scope.starttime = performance.now();
    var usedTime = 0;
    lpl.create("lplTemplate", {lplLoad: "ajax", lplRaw: "lpltask2.json", lplRes: "lplRes", triggerForm: true, lang: "de", view: {icons: false}});
    if(localStorage.getItem('task2') != undefined) {
